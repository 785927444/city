  ...
  <div class="layout-col overlay">
    <div class="hh100 ww100 flex-sc warp">
      ...
      <div class="ww100 flex-cc f18 p40 bot-i16-1 mt20">
        <div class="plr14 ptb5 rad4 ml15 cursor white bgi1 bo-i1-1" @click.stop="emitNext">{{ props.isReadonly ? '下一步' : '下一步' }}</div>
      </div>
    </div>
...
import projects from '@/views/async/actionApply/projects'
...
const onTaskTypeChange = () => {
  ...
  projectWizardService.clearInspectionCache()
}

const onTaskClassChange = (val: any) => {
  publicStore.form.task_content = []
  if (val && val !== '暂无') {
    projectWizardService.fetchInspectionProblems(val, publicStore)
  } else {
    projectWizardService.clearInspectionCache()
  }
}
...
const collect = () => {
  return { form: { ...publicStore.form } }
}

const apply = (data: any) => {
  if (data?.form) Object.assign(publicStore.form, data.form)
}
