<template>
  <el-dialog v-model="state.isFalse" title="批量导入（Excel文件）" :before-close="onVisable" :draggable="true" width="50%">
    ...
    <div class="flex-col-ss flex1" v-if="state.isStatus!='0'">
      ...
      <div v-if="state.isStatus=='1'" class="flex-sc mt10">
        ...
        <div class="ml10">新增成功：<span class="i12 mr5">{{state.addData.length}} 条</span><span :class="state.showStatus=='0'?'bgi1':'bga'" class="mr10 f12 rad2 ptb2 plr5 cursor white" @click.stop="state.showStatus='0'">显示</span></div>
        <div class="ml10">更新成功：<span class="i12 mr5">{{state.updData.length}} 条</span><span :class="state.showStatus=='2'?'bgi1':'bga'" class="mr10 f12 rad2 ptb2 plr5 cursor white" @click.stop="state.showStatus='2'">显示</span></div>
        <div class="ml10">导入失败：<span class="i8 mr5">{{state.errorList.length}} 条</span><span :class="state.showStatus=='1'?'bgi1':'bga'" class="mr10 f12 rad2 ptb2 plr5 cursor white" @click.stop="state.showStatus='1'">显示</span></div>
      </div>
      <div v-if="state.isStatus=='1' && state.showStatus=='0'" class="ww100">
        <div class="ml10 mt10">新增列表：</div>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['init', 'setLists'])
  ...
  const submitUpload = async() => {
    ...
    state.dataList.forEach((item, index) => {
      if(state.form.addItem) Object.assign(item, state.form.addItem)
      if(item.construct_datetime_start && item.construct_datetime_end) {
        item.construct_datetime = `${item.construct_datetime_start}-${item.construct_datetime_end}`
      }
    })
    emit('setLists', state.dataList)
    onVisable()
  }
