...
const setOption = {
	...
	Barsx(attr:any, data:any, pramas:any){
		var extent = attr.extent?attr.extent:4
		var colorList = !isNull(attr.colorList)? attr.colorList : colorLists
		var legendArr = !isNull(attr.legendArr)?attr.legendArr:['']
		var seriesArr = []
		for (var i = 0; i <= attr.legendArr.length-1; i++) {
			let series = {
				name: attr.legendArr[i],type: 'bar',barWidth: newSize(10),connectNulls:true,sampling: 'average',large: true,animation: false,clip: true,
				label: {show: attr.labelShow,position: 'right',color: '#666666',fontSize: newSize(12),fontWeight: 'bold',
					rich:{
						a:{fontSize: newSize(12), padding: [0, 0, 8, 0]},
						b:{fontSize: newSize(12), padding: [0, 0, 8, 0], color: '#FF4D4F'},
						c:{fontSize: newSize(12), padding: [0, 0, 8, 0], color: '#DC1185'},
		...
		let grid = [{top: newSize(40), left: newSize(5), bottom: newSize(10), right: newSize(5), containLabel: true}]
		let tooltip = {show: true, backgroundColor:'rgba(255,255,255,0.95)', textStyle: {color: '#333333'}, trigger: 'axis', axisPointer: {type: 'shadow'},
		formatter: function (p:any) {
			let result = `<div style="font-weight: bold; margin-bottom: 8px; color: #333;">${p[0].value[1]}</div>`;
			p.forEach((item: any) => {
				result += `
					<div style="display: flex; align-items: center; margin: 4px 0;">
						<span style="display: inline-block; width: 10px; height: 10px; border-radius: 50%; background: ${item.color}; margin-right: 8px;"></span>
						<span style="flex: 1; color: #666;">${item.seriesName}:</span>
						<span style="font-weight: bold; color: #333; margin-left: 8px;">${Math.floor((item.value[0]*100))/100} ${attr.tool_dw?attr.tool_dw:''}</span>
					</div>
				`;
			});
			return result;
		...
		let legend = {show: attr.legendShow,data: legendArr,align:'left',x:'right',y: 'top',top: -2,icon: "roundRect",itemWidth: 18,itemHeight: 12,
			textStyle: {color: '#666666',rich: {a: {verticalAlign: 'middle'}},padding:[4,0,0,0]}}
		let dataZoom = [{type: 'inside',show : true,realtime : true,startValue: attr.zoomStart?attr.zoomStart:0,endValue: attr.zoomEnd?attr.zoomEnd:1000}]

		// xAxis现在是value类型，在底部显示数值
		let xAxisShow = attr.xAxisShow===false?false:true;
		let xAxis = [{
			name: attr.y_name,
			nameTextStyle: {color: '#666666',fontSize: newSize(12)},
			type: 'value',
			min: attr.min==0||attr.min?attr.min:0,
			max: attr.max==0||attr.max?attr.max:(value) => {
				return Math.ceil((Math.round(value.max * 10) / 10) + (Math.round(value.max * 10) / 10) / extent)
			},
			//刻度线
			axisTick: {show: false},
			// 轴线
			axisLine: {show: xAxisShow,lineStyle: {color: '#E8E8E8'}},
			// 分割线
			splitLine: {show: true,lineStyle: {color: '#F0F0F0',type: 'dashed'}},
			// 轴文字
			axisLabel: {color: '#666666',fontSize: newSize(12),formatter: '{value}'}
		}]

		// yAxis现在是category类型，在右侧显示地区
		let yAxisShow = attr.yAxisShow===false?false:true;
		let yAxis = {
			type: 'category',
			 //刻度线
			axisTick: {show: false},
			// 轴线
			axisLine: {show: yAxisShow,lineStyle: {color: '#E8E8E8'}},
			// 分割线
			splitLine: {show: false,lineStyle: {color: '#E8E8E8'}},
			// 轴文字
			axisLabel: {color: '#333333',show: true,fontSize: newSize(12),margin: newSize(8)},
			z: 10
		}
		// console.log(seriesArr)
		const option = {
