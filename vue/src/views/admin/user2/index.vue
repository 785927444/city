...
<script lang="ts" setup>
  ...
  const init = async() => {
    state.query = {model: state.model, args: `role_id='5'`, order: `CAST(id AS SIGNED) ASC`}
  ...
  const setInit = async() => {
    state.addItem = {station_num: publicStore.active.id, type: 'enterprise', role_id: '5', status: '0' }
  }
