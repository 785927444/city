<template>
  <div class="layout-col">
    ...
    <div class="ww100 flex-bc h60 mb15">
      <div class="hh100 ww100 rad8 f18 fw flex-cc cursor white relative hidden" @click.stop="toPath('/schemeDesignReport')">
        <img class="ww100 hh100" src="@/assets/images/a_scheme1.png" />
        <span class="absolute-cc z1 ww100 tc">片区策划上报</span>
      </div>
      <!-- <div class="hh100 ww48 rad8 f18 fw flex-cc cursor white relative hidden" @click.stop="toPath('/schemeDesignTrack')">
        <img class="ww100 hh100" src="@/assets/images/a_scheme2.png" />
        <span class="absolute-cc z1 ww100 tc">片区规划跟踪</span>
      </div> -->
    </div>
    <div class="layout-col">
      <!-- 片区策划上报情况 -->
      <div class="ww100 bg-white rad8 p15 mb15">
        <div class="c9 f16 fw mb10">片区策划上报情况</div>
        <div class="ww100 flex-bc i2 f12">
          <div class="flex1 flex-sc">共<span class="mlr4 i1 fw f20">{{ publicStore._public?.data1?.sum ?? '0' }}</span>个上报单位</div>
          <div class="w90 h90 bg-white bo-i1-8 rad50 p8 relative">
            <div class="absolute-cc z1">
              <div class="w60 h60" id="Liquidfill_liqu"></div> 
            </div>
          </div>
          <div class="flex1 flex-ec">有<span class="mlr4 i1 fw f20">{{ publicStore._public?.data1?.release??'0' }}</span>个完成上报</div>
        </div>
      </div>
      <!-- 规划内容 -->
