<template>
  <div class="layout-col">
    ...
    <step111 @init="init" :state="state" ref="addRef" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    export: [
      ...
      { key: 'construct_datetime', name: '建设周期' }, 
  ...
  const init = async(key) => {
    let query = {model: 't_scheme_project', args: `user_id='${configStore.user.id}' and type='design'`}
  ...
  const handleClick = async(remark, val) => {
    ...
    if(remark == 'dels'){
      ...
      if(publicStore._public && !proxy.isNull(publicStore._public.project)){
       const hasIntersection = state.clickArr.some(({id}) => publicStore._public.project.includes(id))
       if(hasIntersection) return ElNotification({ title: '提示', message: '不能删除规划中已存在的项目，请取消后在删除', type: 'error' })
      }
      dels(state.clickArr)
    }
    if(remark == 'active'){
      ...
      const exists = !proxy.isNull(publicStore.form.project)? publicStore.form.project.find(a=>a == val.id) : false
      publicStore.form.project = exists ? publicStore.form.project.filter(a => a !== val.id) : [...publicStore.form.project, val.id]
    }
    if (remark == 'selectAll') {
        ...
        if (hasUnselected) {
            ...
            publicStore.form.project = [...new Set([...publicStore.form.project, ...currentPageIds])]
        } else {
            state.list.forEach(item => delete item.click)
            state.clickArr = state.clickArr.filter(item => !currentPageIds.includes(item.id))
            publicStore.form.project = publicStore.form.project.filter(id => !currentPageIds.includes(id))
        }
    }
    if(remark == 'out'){
      let query = {model: 't_scheme_project', args: `user_id='${configStore.user.id}'`}
