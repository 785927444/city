...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    content: [
      ...
      { width: 'flex1', show: true, align: 'center', key: 'name', name: '文件夹名称' },
      { width: 'w50x4', show: true, align: 'center',   key: 'filetype', name: '文件格式', type: 'select', list: dictStore.fileType, value: 'value', label: 'name' }, 
      { width: 'w50x4', show: true, align: 'center', key: 'filelimit', name: '限制数量' }, 
      { width: 'w50x4', show: true, align: 'right', key: {view: '查看'}, name: '操作' },
    ],
    editFrom: [
      { required: true,  editshow: 'none', name: '上级',      key: 'parent_id', type: 'input' },
      { required: true,  editshow: true,   name: '文件夹名称', key: 'name', type: 'input' },
      { required: false, editshow: true,   name: '标识',      key: 'type', type: 'input' },
      { required: false, editshow: true,   name: '文件格式',  key: 'filetype', type: 'select', list: dictStore.fileType, value: 'value', label: 'name' },
      { required: false, editshow: true,   name: '限制数量',  key: 'filelimit', type: 'input' },
      { required: false, editshow: true,   name: '排序',     key: 'orderd', type: 'input' },
    ],
    addItem: {},
    model: 't_file_content',
  })

  watch(() => publicStore.active, async(val) => {
    if(proxy.isNull(val)) return
    await nextTick()
    state.addItem = {parent_id: val.id, type: val.type}
    await getInit()
    init()
