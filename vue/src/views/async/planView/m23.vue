    ...
    <div class="layout-col">
      ...
      <el-table-column label="中央《意见》任务名称" align="center">
        <template #default="scope, $index">
          <span>{{!isNull(state.task_types)&&scope.row.task_type?find(state.task_types, ['id', scope.row.task_type], 'name'):''}}</span>
        </template>
      </el-table-column>
      <el-table-column label="涉及的重点建设内容" align="center">
        <template #default="scope, $index">
          <span>{{!isNull(state.task_classs)&&scope.row.task_class?find(state.task_classs, ['id', scope.row.task_class], 'name'):''}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="unit_target" label="项目量化目标" width="150" align="center" />
      </el-table>
    </div>
