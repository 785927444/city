...
<script lang="ts" setup>
  ...
  const init = async(key) => {
    let query = key? {model: 't_scheme_plan', args: `id='${key}'`} : {model: 't_scheme_plan', args: `user_id='${configStore.user.id}'`}
    let res = await publicStore.http({Api: query})
    if(!proxy.isNull(res)){
      let data = res[0]
      // 获取关联项目
  ...
  const getProject = (data) => {
    let query = {model: 't_scheme_plan_project', args: `scheme_plan_id='${data.id}'`}
  ...
  const getTask = (data) => {
    let query = {model: 't_scheme_plan_task', args: `scheme_plan_id='${data.id}'`}
    publicStore.http({Api: query}).then(res=>{
      if(!proxy.isNull(res)){
