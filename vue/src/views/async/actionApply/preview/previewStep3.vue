  ...
  <div class="layout-col overlay">
    <div class="hh100 ww100 flex-sc warp">
        ...
        <div class="ww100 white-rgba50 rad8 p15 mt16">
          ...
          <div v-show="tabKey==='key'" class="layout-col white-rgba50 rad8 p15">
            <el-table
              ...
              <el-table-column prop="t2028" label="2028年目标" align="center" width="120" />
            </el-table>
          </div>
        </div>
        <div class="ww100 white-rgba50 rad8 p15 mt16">
      ...
      <div class="ww100 flex-cc f18 p40 bot-i16-1 mt20">
        ...
        <!-- 只有在非基础库查看时，才显示通过/驳回 -->
        <div class="flex-sc" v-else-if="!isBaseLibraryView">
...
<script lang="ts" setup>
  ...
  const isBaseLibraryView = computed(() => {
    // 基础库的路径列表，或者通过 query 参数判断
    // 这里我们假设通过 query.key 或 action 来判断是否是单纯的查看模式
    // actionDepartmentMine, actionDepartment, actionEnterprise 对应的 key 可能是 department 或 enterprise
    // 且如果没有特定的审批 action (如 push_audit 或 reserve_audit)，则认为是基础库查看
    
    // 如果没有 action 参数，通常就是基础查看
    if (!route.query.action) return true
    
    // 或者是特定的“只读/查看” action
    // 这里根据用户描述，如果是从这三个页面进来的，通常是为了查看详情
    // 我们可以结合上一页的路由来判断，但在 Vue 中直接判断 query 更稳妥
    // 假设：如果不是审批操作，就隐藏按钮
    const auditActions = [
      '019c03f8-971e-72a5-8dc3-82f1a1fc2758', // 推送审批
      '019c041a-9f8c-7e9e-91eb-0ae051e6f324', // 储备审批
    ]
    return !auditActions.includes(String(route.query.action))
  })
  ...
  const getDetailData = async () => {
    ...
    publicStore.http(query).then(res => {
      ...
      console.error('previewStep3 加载三表明细失败:', err)
    })
  }
