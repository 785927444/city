          ...
          <div class="ww100 bs bo-i16-1 relative rad8">
            ...
            <div class="ww100 flex-col p12 c6">
              ...
              <div class="ww100 flex-sc mt10">
                ...
                <div class="rad5 ptb5 plr12 cursor bgi1 white" @click.stop="toPath('/actionApply', {key:'enterprise', action: '1f1000c7-5def-66b0-bc96-dc4dc45fb99c', id: v.id})">重新申请</div>
              </div>
            </div>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  const init = async(key) => {
    ...
    let field = ``
    let args = `apply_status='3' and reserve_status='2' and user_id='${configStore.user.id}'`
  ...
  const handleClick = async(remark, val) => {
    if(remark == 'remind') {
      ElMessageBox.confirm('是否确定提醒?', '温馨提示', {confirmButtonText: '确定', cancelButtonText: '关闭', type: 'warn'}).then(async() => { 
