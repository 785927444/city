          ...
          <div class="ww25 cursor p8" v-for="(v, i) in state.list" :key="i">
            <div class="ww100 bs bo-i16-1 relative rad8 project-card" @click.stop="onViewDetail(v)">
              <div class="ww100 flex-sc p12 bob-ce-1">
                ...
                <span class="flex1 flex-ec" @click.stop="onToggleRow(v)">
              ...
              <div v-if="state.tab !== 'enterprise'" class="project-reserve-btn rad5 ptb5 plr12 cursor bgi1 white" @click.stop="onStartReserve(v)">开始储备</div>
            </div>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  const init = async () => {
    ...
    // 获取当前用户级别
    const userLevelStr = configStore.user.level || '1'
    const levelMap = { 'province': 1, 'city': 2, 'district': 3, '1': 1, '2': 2, '3': 3 }
    const currentUserLevel = levelMap[userLevelStr] || 1
    console.log(`[可见性验证] 当前用户级别: ${userLevelStr} (数值: ${currentUserLevel})`)

    // 获取所有满足基础查询条件的项目
    const qAll = Object.assign({}, base, { limit: 1000, page: 1 })
    const resAll: any = await publicStore.http({ Api: qAll })
    let allList = proxy.isNull(resAll) ? [] : resAll

    if (allList.length > 0) {
      const originalCount = allList.length
      // 提取所有项目的 user_id
      const userIds = [...new Set(allList.map(item => item.user_id))]
      // 查询这些用户的 level
      let userQuery = { 
        model: 't_user', 
        args: `id IN ('${userIds.join("','")}')`,
        field: 'id,level'
      }
      let userRes = await publicStore.http({ Api: userQuery })
      const userLevelMap = {}
      if (!proxy.isNull(userRes)) {
        userRes.forEach(u => {
          userLevelMap[u.id] = u.level
        })
      }

      // 过滤项目：
      // 1. 权限可见性过滤：项目创建者级别 >= 当前用户级别 (1省 2市 3区)
      // 2. Tab 业务过滤：企业上报 (level == '0') vs 部门上报 (level != '0')
      allList = allList.filter(item => {
        const projectUserLevelStr = userLevelMap[item.user_id] || '3'
        const projectUserLevelNum = levelMap[projectUserLevelStr] || 3
        
        // 权限检查
        const isVisible = projectUserLevelNum >= currentUserLevel
        if (!isVisible) return false

        // Tab 区分
        if (state.tab === 'enterprise') {
          return projectUserLevelStr === '0'
        } else {
          return projectUserLevelStr !== '0'
        }
      })
      console.log(`[可见性验证] 过滤前总数: ${originalCount}, 过滤后总数: ${allList.length}, 过滤掉: ${originalCount - allList.length}`)
    }

    state.total = allList.length
    
    // 手动分页
    const start = (state.page - 1) * state.limit
    const end = start + state.limit
    state.list = allList.slice(start, end)
  ...
  const onViewDetail = (row: any) => {
    ...
    toPath('/actionApply', { action: '1f10502a-e388-6850-8aa3-265c92863865', mode: 'view', steps: '2', id: row.id || '', project_name: row.name || '' })
  }
  ...
  const onStartReserve = (row: any) => {
    ElMessageBox.confirm('是否将该项目开始储备?', '温馨提示', { confirmButtonText: '确定', cancelButtonText: '关闭', type: 'warn' }).then(() => {
      ...
      toPath('/actionApply', { action: '1f1000c7-5def-66b0-bc96-dc4dc45fb98c', id: row.id || '', project_name: row.name || '' })
    })
  }
