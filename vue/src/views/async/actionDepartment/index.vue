  ...
  <div class="layout-col">
          ...
          <div class="ww100 bs bo-i16-1 relative rad8 project-card" @click.stop="onViewDetail(v)">
            ...
            <div class="ww100 flex-col p12 c6">
              ...
              <div class="ww100 flex-sc mt10">
                ...
                <div class="rad5 ptb5 plr12 cursor bgi1 white" v-if="v.reserve_status=='0' && configStore.user.id == v.user_id" @click.stop="state.active = {...v}; state.isFalse = !state.isFalse">开始储备</div>
                <div class="rad5 ptb5 plr12 cursor bgi1 white" v-if="v.reserve_status>'0' && configStore.user.id == v.user_id" @click.stop="toPath('/actionApply', {key:'department', action: '1f10502a-e388-6850-8aa3-265c92863865', id: v.id})">更新</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <el-dialog v-model="state.isFalse" title="温馨提示" :draggable="true" width="30%">
      <div class="ww100 flex-col-cc">
        <div class="f20 tc ptb30">是否将该项目纳入储备，需补充资料</div>
      </div>
      <template #footer>
        <div class="dialog-footer">
          <el-button size="large" class="bgc white" @click="state.isFalse = !state.isFalse">取 消</el-button>
          <el-button size="large" class="bgi1 white" @click.stop="toPath('/actionApply', {key:'department', action: '1f1000c7-5def-66b0-bc96-dc4dc45fb98d', id: state.active.id})">确 定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
<script lang="ts" setup>
  ...
  const init = async(key) => {
    ...
    let field = ``
    let args = `reserve_status!='1' and user_role>'${configStore.user.role_id}' and user_role<'5'`
  ...
  const onViewDetail = (row: any) => {
    proxy.toPath('/actionApply/preview/previewIndex', { id: row.id, key: 'department' })
  }
</script>
