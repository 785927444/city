...
export const areaOptions = [
  // --- 山西省数据 ---
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { code: '140726', name: '太谷县' }, // 历史兼容，实际已设区
          { code: '140727', name: '祁县' },
	...
	export const setAreaLevel = (type = 'district') => {
  // 1. 定义不同类型对应的深度
  // 1 = 只到省, 2 = 只到市, 3 = 只到区
  let maxDepth = 3
  ...
  const trimData = (list, currentLevel) => {
    return list.map(item => {
      // 浅拷贝当前节点属性，避免修改原始引用数据
      const node = { ...item }
      
      // 如果当前层级小于最大深度，说明还需要显示子级，继续递归
      if (currentLevel < maxDepth && item.children) {
        node.children = trimData(item.children, currentLevel + 1)
      } else {
        // 如果当前层级已经达到限制，删除 children 属性
  ...
  // 3. 【修正点】这里必须使用 areaOptions，而不是 originalAreaData
